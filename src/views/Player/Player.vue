...
export default {
  ...
  computed: {
    percentComplete() {
      return Number.parseInt(this.currentSeconds / this.durationSeconds * 100, 10);
    },
  ...
  methods: {
    ...
    load() {
      if (this.audio.readyState >= 2) {
        this.loaded = true;
        this.durationSeconds = Number.parseInt(this.audio.duration, 10);
    ...
    seek(event) {
      if (!this.playing || event.target.tagName === 'SPAN') {
        return;
      }
      const el = event.target.getBoundingClientRect();
      const seekPos = (event.clientX - el.left) / el.width;

      this.audio.currentTime = Number.parseInt(this.audio.duration * seekPos, 10);
    },
    ...
    update() {
      this.currentSeconds = Number.parseInt(this.audio.currentTime, 10);
    },
  },
